...
cmake -S "${DEPS_DIR}/openmw" -B "${BUILD_DIR}/ios" -G Xcode \
    ...
    -DOPENMW_IOS_VCPKG_LIBRARY_DIR:PATH="${VCPKG_PREFIX}/lib" \
    ...
    -DOPENAL_INCLUDE_DIR:PATH="${VCPKG_PREFIX}/include" \
    -DOPENAL_LIBRARY:FILEPATH="${VCPKG_PREFIX}/lib/libopenal.a" \
    -DOPENMW_IOS=ON \
    ...
    -DHAVE_PTHREAD_YIELD:BOOL=OFF \
    -DHAVE_SCHED_YIELD:BOOL=ON \
